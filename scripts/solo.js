...
let champData = [];
let roleFilter = "";        // 포지션 필터
let queryFilter = "";       // 검색어 필터
let curSet = 1, stageIdx = 0;
...
function autoBanPick() {
  ...
  const candidates = champData
    .map(c => c.key_kr)
    .filter(name => !used.has(name));
  if (candidates.length === 0) return;
  onConfirm(candidates[Math.floor(Math.random() * candidates.length)]);
}
...
// 솔로 모드 초기화
function initSoloMode(data) {
  champData = data.map(r => ({ ...r, pos: r.pos.toLowerCase() }));
  champData.sort((a, b) => a.key_kr.localeCompare(b.key_kr, "ko"));
  for (let i = 0; i < 5; i++) picksBySet[i] = [];
...
function renderGrid(query, role) {
  ...
  champData.forEach(c => {
    ...
    lbl.className = 'champion-name';
    lbl.textContent = c.key_kr;
    item.append(img, lbl);
...
function banPreview(c) {
  if (previewSlot) previewSlot.querySelector('img.preview')?.remove();
...
function pickPreview(c) {
  ...
  for (const b of col.children) {
    if (!b.querySelector('img')) {
      const pi = document.createElement('img');
      pi.src = `../splash/${c.splash}`;
      pi.className = 'preview';
      b.append(pi);
      const pl = document.createElement('div');
      pl.className = 'picked-label';
      pl.textContent = c.key_kr;
      b.append(pl);
      previewSlot = b;
      break;
    }
...
function onConfirm(key) {
  ...
  if (stages[stageIdx].includes('밴')) {
    ...
    if (curSet < 5) {
      const zones = document.querySelectorAll('.fearless-zone');
      const fb = zones[curSet-1].querySelector('.fearless-ban-box:not(:has(img))');
      if (fb) {
        const mark = document.createElement('img');
        mark.src = `../icons/${champData.find(x=>x.key_kr===key).icon}`;
        fb.append(mark);
        const bo = document.createElement('div');
        bo.className='ban-overlay';
        fb.append(bo);
      }
    }
  }
...
function onNextSet() {
  ...
  [bluePickCol, redPickCol].forEach((col,i) => {
    col.innerHTML = '';
    const team = i===0?'블루':'레드';
    ["탑","정글","미드","봇","서폿"].forEach(l => {
      const pb = document.createElement('div');
      pb.className = 'pick-box';
      const lb = document.createElement('div');
      lb.className = 'lane-label';
      lb.textContent = `${team} ${l}`;
      pb.append(lb);
      col.append(pb);
    });
  });
  ...
  curSet++;
  CUR_SET.textContent = curSet;

  renderGrid(queryFilter, roleFilter);
  updateStage();
}

fetchChampions('../data/champion.json')
  .then(initSoloMode)
  .catch(console.error);

export { initSoloMode, updateStage, onConfirm, onNextSet };
